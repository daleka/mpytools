...
async function compileNonPyFileAsAsset(
  ...
  try {
    ...
    const roundTrip = Buffer.from(textPayload, 'utf-8');
    if (roundTrip.equals(source)) {
      const textLiteral = JSON.stringify(textPayload);
      wrapperCode = [
        `# Auto-generated by MPyTools from "${relativeFromSrc}"`,
        `SOURCE_PATH = "${relativeFromSrc}"`,
        `${constantName} = ${textLiteral}`,
        `CONTENT = ${constantName}`,
        '',
        'def get_text():',
        `    return ${constantName}`,
        '',
        'def get_bytes(encoding="utf-8"):',
        `    return ${constantName}.encode(encoding)`,
        ''
      ].join('\n');
    } else {
      throw new Error('non-utf8');
    }
  } catch {
    ...
    wrapperCode = [
      ...
      `SOURCE_PATH = "${relativeFromSrc}"`,
      `${constantName}_B64 = "${payloadB64}"`,
      `${constantName} = _b.a2b_base64(${constantName}_B64)`,
      `DATA = ${constantName}`,
      '',
      'def get_bytes():',
      `    return ${constantName}`,
