...
export function activate(context: vscode.ExtensionContext) {
  ...
  saveProjectStatusBarItem.tooltip = 'Архівувати проект (папка src)';
  ...
  let disposableSaveProject = vscode.commands.registerCommand('mpytools.saveProject', async () => {
    let workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders || workspaceFolders.length === 0) {
      vscode.window.showErrorMessage('Не знайдено відкритий Workspace.');
      return;
    }
    const workspaceRoot = workspaceFolders[0].uri.fsPath;
    const srcFolderPath = path.join(workspaceRoot, 'src');
    if (!fs.existsSync(srcFolderPath)) {
      vscode.window.showErrorMessage('Папка src не існує у Workspace.');
      return;
    }
    ...
    let archiveCommand = '';
    if (os.platform() === 'win32') {
      // Використовуємо PowerShell Compress-Archive; змінюємо робочу директорію на workspaceRoot
      archiveCommand = `powershell -Command "Compress-Archive -Path '${srcFolderPath}' -DestinationPath '${archiveFilePath}'"`;
    } else {
      // Використовуємо команду zip; переходимо в директорію workspaceRoot
      archiveCommand = `cd "${workspaceRoot}" && zip -r "${archiveFilePath}" "src"`;
    }
    try {
      await execPromise(archiveCommand);
      vscode.window.showInformationMessage(`Проект збережено як ${archiveFileName}`);
    } catch (err: any) {
      vscode.window.showErrorMessage(`Помилка при архівації: ${err.message}`);
    }
  });
...
// Допоміжна функція для визначення наступної версії архіву
function getNextVersion(existingArchives: string[]): string {
  ...
  if (patch >= 100) {
    ...
    if (minor >= 100) {
      ...
      major++;
      if (major >= 100) {
        throw new Error('Досягнуто максимальну версію архіву: v_99.99.99');
      }
    }
  }
